...
CREATE POLICY "Users can view match players"
  ...
  USING (
    EXISTS (
      ...
      WHERE m."id" = "matchId"
      AND (m."isPublic" = true OR m."creatorId" = auth.uid()::text OR
           EXISTS (SELECT 1 FROM "MatchPlayer" mp WHERE mp."matchId" = m."id" AND mp."playerId" = auth.uid()::text))
    )
  );
...
CREATE POLICY "Users can update own match player status"
  ON "MatchPlayer"
  FOR UPDATE
  USING (auth.uid()::text = "playerId" OR
         EXISTS (SELECT 1 FROM "Match" WHERE "id" = "matchId" AND "creatorId" = auth.uid()::text))
  WITH CHECK (auth.uid()::text = "playerId" OR
              EXISTS (SELECT 1 FROM "Match" WHERE "id" = "matchId" AND "creatorId" = auth.uid()::text));
...
CREATE POLICY "Users can view group members"
  ...
  USING (
    EXISTS (
      ...
      WHERE g."id" = "groupId"
      AND (g."creatorId" = auth.uid()::text OR
           EXISTS (SELECT 1 FROM "GroupMember" gm WHERE gm."groupId" = g."id" AND gm."userId" = auth.uid()::text))
    )
  );
...
CREATE POLICY "Group creators and admins can add members"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM "Group"
      WHERE "id" = "groupId"
      AND ("creatorId" = auth.uid()::text OR
           EXISTS (SELECT 1 FROM "GroupMember" WHERE "groupId" = "Group"."id" AND "userId" = auth.uid()::text AND "role" = 'admin'))
    )
  );
...
CREATE POLICY "Users can rate players"
  ON "Rating"
  FOR INSERT
  WITH CHECK (auth.uid()::text = "ratedById");

-- Users can update their own ratings
CREATE POLICY "Users can update own ratings"
  ON "Rating"
  FOR UPDATE
  USING (auth.uid()::text = "ratedById")
  WITH CHECK (auth.uid()::text = "ratedById");
...
CREATE POLICY "Authenticated users can create notifications"
  ON "Notification"
  FOR INSERT
  WITH CHECK (auth.uid()::text = "senderId" OR auth.uid() IS NOT NULL);
...
CREATE POLICY "Users can view accessible chat rooms"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM "Match" m
      WHERE m."id" = "matchId"
      AND (m."creatorId" = auth.uid()::text OR
           EXISTS (SELECT 1 FROM "MatchPlayer" mp WHERE mp."matchId" = m."id" AND mp."playerId" = auth.uid()::text AND mp."status" = 'accepted'))
    )
  );
...
CREATE POLICY "Users can view chat room members"
  ...
  USING (
    ...
    EXISTS (
      ...
      WHERE cr."id" = "A"
      AND (
        m."creatorId" = auth.uid()::text OR
        EXISTS (
          SELECT 1 FROM "MatchPlayer" mp
          WHERE mp."matchId" = m."id"
          AND mp."playerId" = auth.uid()::text
