...
export default function HomePage() {
  ...
  const golfCarouselData = [
    ...
    {
      image: "/images/golf_public_background.jpg",
                  ...
                  <div className="relative">
                    ...
                    {totalMyRounds > itemsPerPage && (
                      <div className="flex justify-between items-center mb-4">
                        ...
                        >
                          <FontAwesomeIcon icon={faChevronUp} className="h-4 w-4 text-green-600" />
                        </Button>
                        <span className="text-xs text-gray-500">
                          {myRoundsCarouselIndex + 1}-{Math.min(myRoundsCarouselIndex + itemsPerPage, totalMyRounds)} of {totalMyRounds}
                        </span>
                        <Button
                          variant="ghost"
                          size="sm"
                          ...
                          className="p-2 hover:bg-green-100 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <FontAwesomeIcon icon={faChevronDown} className="h-4 w-4 text-green-600" />
                        </Button>
                      </div>
                    )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div 
                        key={match.id} 
                        className="group relative p-3 sm:p-4 md:p-5 bg-white/95 backdrop-blur-sm border border-green-200/50 rounded-xl shadow-md hover:shadow-xl hover:shadow-green-200/30 transition-all duration-300 cursor-pointer hover:bg-white sm:hover:scale-[1.02] sm:hover:-translate-y-2 [&::before]:hidden [&::after]:hidden"
                      ...
                      >
                          ...
                          <div className="flex items-start space-x-2 sm:space-x-3 w-full sm:w-auto">
                            ...
                            <div className="flex-1">
                              <div className="flex items-start justify-between mb-1">
                                <h3 className="font-bold text-lg text-gray-900 group-hover:text-green-700 transition-colors duration-200">{match.title}</h3>
                              </div>
                              
                              {/* Golf Course Details */}
                              <div className="mb-2">
                                <h4 className="font-semibold text-base text-gray-800 mb-0.5 flex items-center">
                                  <FontAwesomeIcon icon={faLocationDot} className="h-3 w-3 mr-2 text-green-500" />
                                  {match.course}
                                </h4>
                                <div className="ml-5">
                              ...
                              {match.description && (
                                <div className="mb-2">
                                  <p className="text-gray-600 text-xs italic bg-gray-50 p-2 rounded-lg">
                                    "{match.description}"
                                  </p>
                                </div>
                              )}
                          ...
                          <div className="w-full sm:w-auto sm:text-right">
                            ...
                            <div className="flex flex-wrap items-center justify-start sm:justify-end gap-1 sm:gap-2 text-xs">
                              <span className="flex items-center text-gray-600">
                                <FontAwesomeIcon icon={faCalendarDays} className="h-2 w-2 mr-1 text-blue-500" />
                                {formatDate(match.date)}
                              </span>
                              <span className="flex items-center text-gray-600">
                                <svg className="h-2 w-2 mr-1 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            {(match.pendingRequestsCount ?? 0) > 0 && (
                              <div className="mt-1 flex justify-end">
                                <div 
                                  className="bg-red-500 text-white px-2 py-0.5 rounded-full text-xs font-bold shadow-lg animate-pulse"
                                >
                                  Pending Player
                                </div>
                              </div>
                        ...
                        {match.courseFeatures && (
                          <div className="mt-3 pt-2 border-t border-green-100/50">
                            <p className="text-green-700 text-xs font-medium bg-green-50 p-2 rounded-lg border border-green-200 flex items-center">
            ...
            <Card className="bg-gradient-to-br from-white/95 to-purple-50/80 backdrop-blur-md shadow-xl border border-purple-200/30 hover:shadow-2xl transition-all duration-300 rounded-2xl">
              ...
              <CardContent>
                {myGroups && myGroups.length > 0 ? (
                  <div className="space-y-4">
                    {myGroups.map((group) => {
                      const isOwner = group.creator.id === session?.user?.id
                      return (
                        <div 
                          key={group.id} 
                        ...
                        >
                          <div className="flex items-center space-x-3 sm:space-x-4 w-full sm:w-auto">
                            <div className={`p-2 sm:p-3 rounded-2xl shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all duration-300 ${
                              isOwner 
