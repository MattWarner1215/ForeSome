...
import { prefetchCommonQueries, invalidateMatchQueries } from '@/lib/query-invalidation'
...
export default function HomePage() {
  ...
  const leaveRound = useMutation({
    ...
    onMutate: async (matchId: string) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['recent-rounds'] })

      // Snapshot the previous value for rollback
      const previousRounds = queryClient.getQueryData<Match[]>(['recent-rounds'])
      ...
      if (previousRounds && session?.user?.id) {
        ...
        queryClient.setQueryData(['recent-rounds'], updatedRounds)
      }
    ...
    onError: (err, matchId, context) => {
      ...
      if (context?.previousRounds) {
        queryClient.setQueryData(['recent-rounds'], context.previousRounds)
      }
    },
    onSuccess: () => {
      ...
      queryClient.refetchQueries({ queryKey: ['recent-rounds'] })
    }
  })
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {visibleMyRounds.map((round) => (
                        <EnhancedRoundCard
                          ...
                          onLeaveClick={() => leaveRound.mutate(round.id)}
                          onManageClick={() => router.push(`/matches/${round.id}/manage`)}
                          isLoading={leaveRound.isPending}
                        />
                      ))}
                    </div>
                  </div>
